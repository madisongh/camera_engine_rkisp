...
$(EXECUTABLE_TARGET): EXECUTABLE_TARGET_LDFLAG += -L$(BUILD_OUTPUT_STATIC_LIBS) $(EXECUTABLE_TARGET_SHARED_LIBRARIES) $(EXECUTABLE_TARGET_STATIC_LIBRARIES) -L$(BUILD_OUTPUT_RKISP_LIBS) -lstdc++ -ldl -lm -lpthread -Xlinker --unresolved-symbols=ignore-in-shared-libs

all: $(EXECUTABLE_TARGET)
$(EXECUTABLE_TARGET):$(EXECUTABLE_TARGET_C_OBJ) $(EXECUTABLE_TARGET_CPP_OBJ)
	@mkdir -p $(dir $@)
	$(Q)$(TARGET_GPP) -o  $@ $^ $(EXECUTABLE_TARGET_LDFLAG)
	$(call quiet-cmd-echo-build, G++EXE, $@)
$(EXECUTABLE_TARGET_C_OBJ):$(LOCAL_BUILD_DIR)/%.o:%.c
	@mkdir -p $(dir $@)
	$(Q)$(TARGET_GCC) $(EXECUTABLE_TARGET_CFLAG) $(EXECUTABLE_TARGET_FLAG) -c $< -o $@
	$(call quiet-cmd-echo-build, GCC, $@)
$(EXECUTABLE_TARGET_CPP_OBJ):$(LOCAL_BUILD_DIR)/%.o:%.cpp
	@mkdir -p $(dir $@)
	$(Q)$(TARGET_GPP) $(EXECUTABLE_TARGET_CPPFLAG) $(EXECUTABLE_TARGET_FLAG) -c -fpic $< -o $@
